...
const EditProduct = () => {
  ...
  const getProduct = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/products/edit/${id}`, { method: "GET" });
      if (!res.ok) {
      ...
      setLowStockAt(data.lowStockAt ?? 0);
    } catch (err) {
  ...
  // onsubmit function
    const handleSubmit = async (e:React.FormEvent)=>{
      e.preventDefault()
     const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/products/update/${id}`,{
      method:"PUT",
